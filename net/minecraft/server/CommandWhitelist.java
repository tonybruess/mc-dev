    ...
    public void b(ICommandListener icommandlistener, String[] astring) {
        if (astring.length >= 1) {
            ...
            if (astring[0].equals("list")) {
                icommandlistener.ICommandListener(ChatMessageComponent.b("commands.whitelist.list", new Object[] { Integer.valueOf(MinecraftServer.getServer().getPlayerList().getWhitelisted().size()), Integer.valueOf(MinecraftServer.getServer().getPlayerList().getSeenPlayers().length)}));
                Set set = MinecraftServer.getServer().getPlayerList().getWhitelisted();

                icommandlistener.ICommandListener(ChatMessageComponent.d(a(set.toArray(new String[set.size()]))));
                return;
            }
