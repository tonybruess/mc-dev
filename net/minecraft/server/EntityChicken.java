...
public class EntityChicken extends EntityAnimal {
    ...
    public void c() {
        ...
        if (!this.onGround && this.persistent < 1.0F) {
            this.persistent = 1.0F;
        }

        this.persistent = (float) ((double) this.persistent * 0.9D);
        if (!this.onGround && this.motY < 0.0D) {
            this.motY *= 0.6D;
        }

        this.love += this.persistent * 2.0F;
